...
-- Volcando datos para la tabla quizz_app.marcado: ~4 rows (aproximadamente)
...
-- Volcando datos para la tabla quizz_app.respuesta: ~43 rows (aproximadamente)
...
-- Volcando datos para la tabla quizz_app.resultado: ~15 rows (aproximadamente)
...
-- Volcando datos para la tabla quizz_app.sugerencia: ~15 rows (aproximadamente)
...
CREATE TABLE IF NOT EXISTS `usuario` (
  ...
  KEY `fk_responsable_pk_usuario` (`id_usuario_responsable`),
  CONSTRAINT `fk_responsable_pk_usuario` FOREIGN KEY (`id_usuario_responsable`) REFERENCES `usuario` (`idusuario`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish2_ci;
